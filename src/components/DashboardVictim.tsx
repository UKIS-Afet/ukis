...
export function DashboardVictim({ container, products, onProfileUpdate }: Props) {
  ...
  const requestProduct = async (product: Product, maxAllowed: number) => {
    try {
  ...
  const submitIssue = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIssueSpecial('');
      setIssueComplaint('');
      setIssueMessage({ type: 'success', text: "Talebiniz yetkililere iletildi." });
    } catch (e: any) {
    ...
    <div className="space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {products
                ...
                return (
                  <div key={product.id} className={`bg-white rounded-3xl border transition-all flex flex-col ${cd.available && !locked ? 'border-slate-100 shadow-sm hover:-translate-y-1 hover:shadow-xl' : 'border-slate-100 bg-slate-50 opacity-90'}`}>
                    <div className="relative h-48 w-full overflow-hidden bg-slate-100">
                      <img src={product.imageUrl} className={`w-full h-full object-cover transition-transform duration-500 hover:scale-110 ${cd.available && !locked ? '' : 'grayscale brightness-75'}`} alt={product.name} referrerPolicy="no-referrer" />
        ...
        <form onSubmit={updateProfile} className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <label className="flex flex-col">
              <span className="text-xs font-bold text-slate-500 dark:text-slate-300 mb-1">Yetişkin Sayısı (Sadece Yetkili Değiştirebilir)</span>
            ...
            <label className="flex flex-col">
              <span className="text-xs font-bold text-slate-500 dark:text-slate-300 mb-1">Çocuk Sayısı (Sadece Yetkili Değiştirebilir)</span>
            ...
            <label className="flex flex-col">
              <span className="text-xs font-bold text-slate-500 dark:text-slate-300 mb-1">Bebek Sayısı (Sadece Yetkili Değiştirebilir)</span>
      ...
      {tab === 'issues' && (
        <form onSubmit={submitIssue} className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-6">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Bu kısımdan özel durumlarınızı ve şikayetlerinizi doğrudan yetkililere iletebilirsiniz.</p>
          ...
          <button type="submit" className="w-full bg-red-600 text-white font-bold p-4 rounded-xl hover:bg-red-700">Yetkiliye İlet</button>
        </form>
      )}
    </div>
  );
